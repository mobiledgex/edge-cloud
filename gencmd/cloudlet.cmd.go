...
var StopCrmServiceCmd = &cli.Command{
	Use:          "StopCrmService",
	...
	ReqData:      &edgeproto.Cloudlet{},
	ReplyData:    &edgeproto.Result{},
	Run:          runStopCrmService,
}

func runStopCrmService(c *cli.Command, args []string) error {
	if cli.SilenceUsage {
		c.CobraCmd.SilenceUsage = true
	}
	obj := c.ReqData.(*edgeproto.Cloudlet)
	...
	return StopCrmService(c, obj)
}

func StopCrmService(c *cli.Command, in *edgeproto.Cloudlet) error {
	if CloudletApiCmd == nil {
		return fmt.Errorf("CloudletApi client not initialized")
	}
	ctx := context.Background()
	stream, err := CloudletApiCmd.StopCrmService(ctx, in)
	if err != nil {
		errstr := err.Error()
		st, ok := status.FromError(err)
		if ok {
		...
		return fmt.Errorf("StopCrmService failed: %s", errstr)
	}
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("StopCrmService recv failed: %s", errstr)
		}
		objs = append(objs, obj)
	}
	if len(objs) == 0 {
...
// this supports "Create" and "Delete" commands on ApplicationData
func StopCrmServices(c *cli.Command, data []edgeproto.Cloudlet, err *error) {
	if *err != nil {
		return
	}
	for ii, _ := range data {
		fmt.Printf("StopCrmService %v\n", data[ii])
		myerr := StopCrmService(c, &data[ii])
		if myerr != nil {
			*err = myerr
			break
		}
