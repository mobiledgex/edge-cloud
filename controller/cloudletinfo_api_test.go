...
func TestFlavorInfoDelta(t *testing.T) {
	//func testFlavorInfoDelta(t *testing.T, ctx context.Context, apis *AllApis) {
	...
	addedFlavors, deletedFlavors, updatedFlavors, _ := apis.cloudletInfoApi.findFlavorDeltas(ctx, flavorMap, newFlavorMap)
...
func cpFlavors(flavors []*edgeproto.FlavorInfo, cloudletInfo *edgeproto.CloudletInfo) {
	cloudletInfo.Flavors = nil
	cloudletInfo.Flavors = make([]*edgeproto.FlavorInfo, len(flavors))
	for ii, f := range flavors {
		cloudletInfo.Flavors[ii] = &edgeproto.FlavorInfo{
			Name:    f.Name,
			Vcpus:   f.Vcpus,
			Ram:     f.Ram,
			Disk:    f.Disk,
...
func TestFlavorInfoUpdate(t *testing.T) {
	...
	//_, err = apis.cloudletInfoApi.InjectCloudletInfo(ctx, &cldInfo)
	//require.Nil(t, err)

	//responder := NewDummyInfoResponder(apis.appInstApi.cache, &pis.clusterInstApi.cache,
	...
	err := apis.clusterInstApi.CreateClusterInst(&pokeClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "create poke ClusterInst")
	check := edgeproto.ClusterInst{}
	foundclust := apis.clusterInstApi.cache.Get(&pokeClust.Key, &check)
	...
	err = apis.clusterInstApi.CreateClusterInst(&everAiClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "create everAi ClusterInst")
	check = edgeproto.ClusterInst{}
	foundclust = apis.clusterInstApi.cache.Get(&everAiClust.Key, &check)
	require.True(t, foundclust)

	reserveClust := testutil.ClusterInstData[7]
	reserveClust.CrmOverride = edgeproto.CRMOverride_IGNORE_CRM_ERRORS
	err = apis.clusterInstApi.CreateClusterInst(&reserveClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "create reservable  ClusterInst")
	check = edgeproto.ClusterInst{}
	foundclust = apis.clusterInstApi.cache.Get(&reserveClust.Key, &check)
	require.True(t, foundclust)
	...
	err = apis.appInstApi.CreateAppInst(&pokeInst, testutil.NewCudStreamoutAppInst(ctx))
	...
	sync.ApplySTMWait(ctx, func(stm concurrency.STM) error {
		apis.cloudletRefsApi.store.STMPut(stm, &cldRefs)
		return nil
	})
	...
	sync.ApplySTMWait(ctx, func(stm concurrency.STM) error {
		countRefs, e := apis.cloudletInfoApi.getInfraFlavorUsageCounts(ctx, stm, &cloudletInfo)
		require.Nil(t, e, "get usage count failed %s", e)
		require.Equal(t, 6, countRefs["flavor.tiny2"].refCount, "use count off for flavor.tiny2")
		require.Equal(t, 4, countRefs["flavor.medium"].refCount, "use count off for flavor.medium")
		return nil
	})

	// First an empy update, make sure nothing changes
	cpFlavors(newFlavors, &cloudletInfo)
	...
	require.Equal(t, 12, len(cloudletInfo.Flavors), "tiny2 should be present but deprecated")
	...
	alertFound := false
	alerts := 0
	for k, _ := range apis.alertApi.cache.Objs {
	...
	match, err := apis.cloudletApi.FindFlavorMatch(ctx, &fm)
	...
	sync.ApplySTMWait(ctx, func(stm concurrency.STM) error {
		countRefs, err := apis.cloudletInfoApi.getInfraFlavorUsageCounts(ctx, stm, &cloudletInfo)
		require.Nil(t, err, "getInforaFlavorUsageCounts failed")
		tiny1 := countRefs["flavor.tiny1"]
		require.Nil(t, tiny1, "flavor.tiny1 is unexpectedly in use! test_data?")
		return nil
	})
	...
	apis.cloudletInfoApi.Update(ctx, &cloudletInfo, 0)
	sync.ApplySTMWait(ctx, func(stm concurrency.STM) error {
		found = apis.cloudletInfoApi.store.STMGet(stm, &cloudletInfo.Key, &cloudletInfo)
	...
	require.Equal(t, 10, len(newFlavors), "failed to remove tiny2 from newFlavors!")
	// second in use flavor deleted
	for ii, flavor := range newFlavors {
		if flavor.Name == "flavor.medium" {
			newFlavors[ii] = newFlavors[len(newFlavors)-1]
			newFlavors[len(newFlavors)-1] = &edgeproto.FlavorInfo{}
			newFlavors = newFlavors[:len(newFlavors)-1]
		}
	}
	require.Equal(t, 9, len(newFlavors), "newFlav len off")
	...
	for _, flavor := range cloudletInfo.Flavors {
		if flavor.Name == "flavor.tiny2" {
			require.Equal(t, true, flavor.Deprecated, "tiny2 not deprecated")
		}
		if flavor.Name == "flavor.medium" {
			require.Equal(t, true, flavor.Deprecated, "medium not deprecated")
		}
	...
	// XXX check for both  and the names of the infra flavor in the alerts XXX
	// Also, verify the usage clusters are in the alert / format XXXX
	alertFound = false
	alerts = 0
	for k, _ := range apis.alertApi.cache.Objs {
		if strings.Contains(k.GetKeyString(), "InfraFlavorDeleted") {
	...
	require.Equal(t, 1, alerts, "Alert count wrong")
	require.True(t, alertFound, "failed to find infra flavor deleted alert flavor.tiny2")
	alertFound = false
	alerts = 0
	for k, _ := range apis.alertApi.cache.Objs {
		if strings.Contains(k.GetKeyString(), "InfraFlavorDeleted") {
			if strings.Contains(k.GetKeyString(), "flavor.medium") {
				alertFound = true
				alerts++
			}
		}
	...
	require.Equal(t, 1, alerts, "Alert count wrong")
	require.True(t, alertFound, "failed to find infra flavor deleted alert flavor.medium")

	// test adding a couple of new flavors
	newFlavor1 := edgeproto.FlavorInfo{
		Name:  "newFlavor1",
		Vcpus: uint64(10),
		Ram:   uint64(4096),
	...
	newFlavors = append(newFlavors, &newFlavor1)
	newFlavors = append(newFlavors, &newFlavor2)
	cpFlavors(newFlavors, &cloudletInfo)
	apis.cloudletInfoApi.Update(ctx, &cloudletInfo, 0)

	found = false
	for _, f := range cloudletInfo.Flavors {
		if f.Name == "newFlavor1" {
			found = true
			break
		}
	}
	require.Equal(t, true, found, "add newFlavor1 failed")
	found = false
	for _, f := range cloudletInfo.Flavors {
		if f.Name == "newFlavor2" {
			found = true
			break
		}
	}
	require.Equal(t, true, found, "add newFlavor2 failed")

	// Now we start exercising the worker task called from the using objects as they are deleted
	err = apis.appInstApi.DeleteAppInst(&pokeInst, testutil.NewCudStreamoutAppInst(ctx))
	require.Nil(t, err, "Error Delete poke AppInst")

	err = apis.clusterInstApi.DeleteClusterInst(&pokeClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "delete poke ClusterInst")
	check = edgeproto.ClusterInst{}
	foundclust = apis.clusterInstApi.cache.Get(&pokeClust.Key, &check)
	require.False(t, foundclust)
	everAiClust = testutil.ClusterInstData[3]
	err = apis.clusterInstApi.DeleteClusterInst(&everAiClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "delete everAi ClusterInst")
	check = edgeproto.ClusterInst{}
	foundclust = apis.clusterInstApi.cache.Get(&everAiClust.Key, &check)
	require.False(t, foundclust)
	reserveClust = testutil.ClusterInstData[7]
	err = apis.clusterInstApi.DeleteClusterInst(&reserveClust, testutil.NewCudStreamoutClusterInst(ctx))
	require.Nil(t, err, "delete reservable  ClusterInst")
	check = edgeproto.ClusterInst{}
	foundclust = apis.clusterInstApi.cache.Get(&reserveClust.Key, &check)
	require.False(t, foundclust)
	// wait for the task to complete
	apis.cloudletInfoApi.infraFlavorAlertTask.WaitIdle()
	...
	require.Equal(t, 11, len(cloudletInfo.Flavors), "cloudletInfoFlavors count")
	// finsh clean up
	_, err = apis.cloudletInfoApi.EvictCloudletInfo(ctx, &cloudletInfo)
	// More to cleanup? XXX
