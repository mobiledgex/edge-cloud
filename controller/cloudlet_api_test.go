...
func TestShowCloudletsAppDeploy(t *testing.T) {
	...
	// TODO: Increase Refs such that San Jose Site no longer supports App
	// What it is now:
	// resUsage := getMetricCounts(t, ctx, &testutil.CloudletData[1])
	// CloudletQuota
	// Need the platformType from the cloudlet (FAKE) San Jose = CloudletData[0]
	pfType := testutil.CloudletData[0].PlatformType
	var resProps *edgeproto.CloudletResourceQuotaProps
	cloudletPlatform, err := pfutils.GetPlatform(ctx, pfType.String(), nodeMgr.UpdateNodeProps)
	require.Nil(t, err, "GetPlatform")

	resProps, err = cloudletPlatform.GetCloudletResourceQuotaProps(ctx)
	require.Nil(t, err, "GetCloudletResourceQuotaProps")

	cldResources, err := cloudletPlatform.GetCloudletInfraResources(ctx)
	require.Nil(t, err, "GetCloudletInfraResources")
	fmt.Printf("\n\nCloudletInfraResources %+v\n\n", cldResources)

	//	addProps, err := cloudletPlatform.GetClusterAdditionalResources(ctx,
	//	require.Nil(t, err, "GetCloudletResourceQuota")
	fmt.Printf("\n\nQuotas for San Jose: %+v\n\n", resProps)
}
