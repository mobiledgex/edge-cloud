...
func testAppInstId(t *testing.T, ctx context.Context) {
	var err error
	...
	expId0 := "nianticincapp110-pokemons-sanjosesite-attinc"
	expId1 := "nianticincapp110-pokemons-sanjosesite-attinc-1"

	_, err = appApi.CreateApp(ctx, &app0)
	require.Nil(t, err)
	_, err = appApi.CreateApp(ctx, &app1)
	require.Nil(t, err)

	err = appInstApi.CreateAppInst(&appInst0, testutil.NewCudStreamoutAppInst(ctx))
	require.Nil(t, err)
	err = appInstApi.CreateAppInst(&appInst1, testutil.NewCudStreamoutAppInst(ctx))
	require.Nil(t, err)

	aiCheck0 := edgeproto.AppInst{}
	require.True(t, appInstApi.cache.Get(&appInst0.Key, &aiCheck0))
	require.Equal(t, expId0, aiCheck0.UniqueId)

	aiCheck1 := edgeproto.AppInst{}
	require.True(t, appInstApi.cache.Get(&appInst1.Key, &aiCheck1))
	...
	hasIds := func(hasId0, hasId1 bool) {
		found0 := testHasAppInstId(appInstApi.sync.store, expId0)
		require.Equal(t, hasId0, found0, "has id %s", expId0)
		found1 := testHasAppInstId(appInstApi.sync.store, expId1)
	...
	err = appInstApi.DeleteAppInst(&appInst0, testutil.NewCudStreamoutAppInst(ctx))
	require.Nil(t, err)
	hasIds(false, true)
	err = appInstApi.DeleteAppInst(&appInst1, testutil.NewCudStreamoutAppInst(ctx))
	require.Nil(t, err)
	hasIds(false, false)

	// clean up
	_, err = appApi.DeleteApp(ctx, &app0)
	require.Nil(t, err)
	_, err = appApi.DeleteApp(ctx, &app1)
	require.Nil(t, err)
}
