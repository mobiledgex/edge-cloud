...
func TestAutoClusterInst(t *testing.T) {
	...
	// Stream should not be empty, as object is present in etcd
	msgs := GetAppInstStreamMsgs(t, ctx, &autoDeleteAppInst.Key, apis, Pass)
	require.Greater(t, len(msgs), 0, "progress messages")
...
func testSingleKubernetesCloudlet(t *testing.T, ctx context.Context, apis *AllApis) {
	...
	appInstCreateTests := []struct {
		...
		"pokemongo100-nianticinc.singlek8smt-unittest.local.mobiledgex.net", PASS,
	}, {
...
func testAppInstId(t *testing.T, ctx context.Context, apis *AllApis) {
	...
	dnsLabel0 := "app110-nianticinc"
	dnsLabel1 := "app110-nianticinc1"

	clDnsLabel0 := "app110-nianticinc2"
	clDnsLabel1 := "app110-nianticinc3"
