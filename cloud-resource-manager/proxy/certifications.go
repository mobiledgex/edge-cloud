...
var selfSignedCmd = `openssl req -new -newkey rsa:4096 -nodes -days 90 -nodes -x509 -config <(
cat <<-EOF
[req]
prompt = no
...
var selfSignedCmdWithSAN = `openssl req -new -newkey rsa:4096 -nodes -days 90 -nodes -x509 -config <(
cat <<-EOF
[req]
prompt = no
