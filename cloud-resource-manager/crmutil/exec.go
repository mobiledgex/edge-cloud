...
func (cd *ControllerData) ProcessExecReq(ctx context.Context, req *edgeproto.ExecRequest) error {
	...
	if req.Cmd != nil && req.Cmd.Console {
		req.Cmd.ConsoleUrl, err = cd.platform.GetConsoleUrl(ctx, &app)
		if err != nil {
			return err
		}
	} else {
...
func (s *WebrtcExec) RTCTunnel(d *webrtc.DataChannel) {
	...
	d.OnOpen(func() {
		urlObj, err := url.Parse(s.req.Cmd.ConsoleUrl)
		if err != nil {
			log.DebugLog(log.DebugLevelApi, "failed to parse console url", "url", s.req.Cmd.ConsoleUrl, "err", err)
			return
		}
