...
// given the ports and a string representation of ports to skip,
// set HealthCheck on ports correctly
func SetPortsHealthCheck(ports []dme.AppPort, skipHcPorts string) ([]dme.AppPort, error) {
	noHc, err := ParseAppPorts(skipHcPorts)
	if err != nil {
		return ports, err
	}
	for _, noHcPort := range noHc {
		// make everything a range for the checks
		if noHcPort.EndPort == 0 {
			noHcPort.EndPort = noHcPort.InternalPort
		}
		for ii, port := range ports {
			...
			if noHcPort.InternalPort <= port.InternalPort &&
				noHcPort.EndPort >= port.EndPort {
				// exact match, or port range is fully contained in the
				// excluded range
				ports[ii].SkipHealthCheck = true
			} else if noHcPort.InternalPort <= port.InternalPort &&
				noHcPort.EndPort >= port.InternalPort &&
				noHcPort.EndPort < port.EndPort {
				// skipped ports range starts before app port range
				// split app port range into two ranges
				newPort := port
				newPort.InternalPort = noHcPort.EndPort + 1
				newPort.SkipHealthCheck = false
				ports[ii].EndPort = noHcPort.EndPort
				ports[ii].SkipHealthCheck = true
				ports = append(ports, newPort)
			} else if noHcPort.InternalPort > port.InternalPort &&
				noHcPort.InternalPort <= port.EndPort &&
				noHcPort.EndPort >= port.EndPort {
				// skipped ports range starts after app port range
				// split app port range into two ranges
				newPort := port
				newPort.InternalPort = noHcPort.InternalPort
				newPort.SkipHealthCheck = true
				ports[ii].EndPort = noHcPort.InternalPort - 1
				ports[ii].SkipHealthCheck = false
				ports = append(ports, newPort)
			} else if noHcPort.InternalPort > port.InternalPort &&
				noHcPort.EndPort < port.EndPort {
				// app port range is a superset of skip health check range
				// split into three ports
				startPort := port
				startPort.EndPort = noHcPort.InternalPort - 1
				startPort.SkipHealthCheck = false
				ports = append(ports, startPort)
				endPort := port
				endPort.InternalPort = noHcPort.EndPort + 1
				endPort.SkipHealthCheck = false
				ports = append(ports, endPort)

				ports[ii].InternalPort = noHcPort.InternalPort
				ports[ii].EndPort = noHcPort.EndPort
				ports[ii].SkipHealthCheck = true
			} else {
				ports[ii].SkipHealthCheck = false
	...
	// fix up all the single port ranges into a single ports
	for ii, _ := range ports {
		if ports[ii].InternalPort == ports[ii].EndPort {
			ports[ii].EndPort = 0
		}
	}
	return ports, nil
