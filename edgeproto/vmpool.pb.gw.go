...
func request_VMPoolInfoApi_ShowVMPoolInfo_0(ctx context.Context, marshaler runtime.Marshaler, client VMPoolInfoApiClient, req *http.Request, pathParams map[string]string) (VMPoolInfoApi_ShowVMPoolInfoClient, runtime.ServerMetadata, error) {
	var protoReq VMPoolInfo
	var metadata runtime.ServerMetadata
	...
	stream, err := client.ShowVMPoolInfo(ctx, &protoReq)
	if err != nil {
		return nil, metadata, err
	}
	header, err := stream.Header()
...
// RegisterVMPoolInfoApiHandlerFromEndpoint is same as RegisterVMPoolInfoApiHandler but
// automatically dials to "endpoint" and closes the connection when "ctx" gets done.
func RegisterVMPoolInfoApiHandlerFromEndpoint(ctx context.Context, mux *runtime.ServeMux, endpoint string, opts []grpc.DialOption) (err error) {
	conn, err := grpc.Dial(endpoint, opts...)
	if err != nil {
		return err
	}
	...
	return RegisterVMPoolInfoApiHandler(ctx, mux, conn)
}

// RegisterVMPoolInfoApiHandler registers the http handlers for service VMPoolInfoApi to "mux".
// The handlers forward requests to the grpc endpoint over "conn".
func RegisterVMPoolInfoApiHandler(ctx context.Context, mux *runtime.ServeMux, conn *grpc.ClientConn) error {
	return RegisterVMPoolInfoApiHandlerClient(ctx, mux, NewVMPoolInfoApiClient(conn))
}

// RegisterVMPoolInfoApiHandlerClient registers the http handlers for service VMPoolInfoApi
// to "mux". The handlers forward requests to the grpc endpoint over the given implementation of "VMPoolInfoApiClient".
// Note: the gRPC framework executes interceptors within the gRPC handler. If the passed in "VMPoolInfoApiClient"
// doesn't go through the normal gRPC flow (creating a gRPC client etc.) then it will be up to the passed in
// "VMPoolInfoApiClient" to call the correct interceptors.
func RegisterVMPoolInfoApiHandlerClient(ctx context.Context, mux *runtime.ServeMux, client VMPoolInfoApiClient) error {

	mux.Handle("POST", pattern_VMPoolInfoApi_ShowVMPoolInfo_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(req.Context())
		defer cancel()
		inboundMarshaler, outboundMarshaler := runtime.MarshalerForRequest(mux, req)
		rctx, err := runtime.AnnotateContext(ctx, mux, req)
		...
		resp, md, err := request_VMPoolInfoApi_ShowVMPoolInfo_0(rctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
		...
		forward_VMPoolInfoApi_ShowVMPoolInfo_0(ctx, mux, outboundMarshaler, w, req, func() (proto.Message, error) { return resp.Recv() }, mux.GetForwardResponseOptions()...)
...
var (
	pattern_VMPoolInfoApi_ShowVMPoolInfo_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1}, []string{"show", "vmpoolinfo"}, ""))
)

var (
	forward_VMPoolInfoApi_ShowVMPoolInfo_0 = runtime.ForwardResponseStream
)
